...
4. **Execution** (Layer 4) — **COMPLETE**
   ...
   `CPMTracker` (`src/talos/cpm.py`): per-(ticker, outcome, book_side, price_bps) flow tracking with three-level fallback in `eta_minutes` (exact bucket → drop price → drop book_side). Each Kalshi trade decomposes into TWO `FlowKey` events using the complement rule (`taker_side == outcome → ASK; else → BID`). Backward-compatible `eta_minutes(ticker, queue_position)` shape preserved for legacy callers; new callers pass `outcome / book_side / price_bps` for per-side fill ETA. Bare-ticker aggregate intentionally iterates `outcome=="yes"` only to avoid double-counting (each trade is in both buckets) — see [[decisions#2026-04-27 — CPM/ETA per-side granularity]].
