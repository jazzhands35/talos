...
- **Kalshi uses American spelling `canceled`:** Status field returns `"canceled"` (one L), not `"cancelled"`. Match both in lookup tables.
...
- **Textual 8 breaking changes:** `Static.content` returns text (not `.renderable`). Modal screen checks use `isinstance(app.screen, ScreenClass)` not `app.query()`. Import `work` from `from textual import work`.
